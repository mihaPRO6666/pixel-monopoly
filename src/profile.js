...
class ProfileManager {
    ...
    if (saved) {
      try {
        ...
        if (isDevUser(parsed) || (typeof localStorage !== 'undefined' && localStorage.getItem('monopoly_is_creator') === 'true')) {
          if (!parsed.unlockedTitles.includes('creator')) {
            parsed.unlockedTitles.push('creator');
          }
        }
  ...
  updateName(newName) {
    if (!this.isRegisteredUser()) return; // Nickname is only set after logging in through Discord
