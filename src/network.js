...
export class NetworkManager {
  ...
  listenForPublicLobbies(onUpdate) {
    ...
    this.discoveryChannel.on('broadcast', { event: 'LOBBY_ANNOUNCE' }, ({ payload }) => {
      if (payload && payload.roomCode) {
        ...
        if (payload.status === 'CLOSED') {
          ...
          if (payload.hostId && payload.hostName && (payload.isRegistered || payload.discordId)) {
            import('./leaderboard.js').then(({ leaderboardManager }) => {
    ...
    this.discoveryChannel.on('broadcast', { event: 'LEADERBOARD_ANNOUNCE' }, ({ payload }) => {
      if (payload && payload.id && (payload.isRegistered || payload.discordId)) {
    ...
    // When a new user connects and requests leaderboard, all registered users respond with their record
    this.discoveryChannel.on('broadcast', { event: 'REQUEST_LEADERBOARD' }, () => {
      if (isTestOrLocalEnvironment(this.roomCode)) return;
      import('./leaderboard.js').then(({ leaderboardManager }) => {
        import('./profile.js').then(({ profileManager }) => {
    ...
    this.discoveryChannel.subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        ...
        // Request existing leaderboard records from all connected registered users
        setTimeout(() => {
          this.discoveryChannel.send({
            type: 'broadcast',
  ...
  broadcastLeaderboardRecord(record) {
    if (!this.discoveryChannel || !record || (!record.isRegistered && !record.discordId)) return;
