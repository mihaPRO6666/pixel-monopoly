...
import { ui, showToast, showConfirm } from './ui.js';
import { network } from './network.js';
import { sound } from './audio.js';
import { themeManager, THEMES } from './theme.js';
import { profileManager, AVAILABLE_TOKENS, PLAYER_COLORS, PROFILE_BACKGROUNDS, getProfileBg, getTokenEmoji, getTokenName, renderTokenHTML, isDevUser } from './profile.js';
import { GAME_PRESETS, getPresetById } from './presets.js';
import { BOARD_TILES, COLOR_GROUPS } from './board-data.js';
import { leaderboardManager, isPlayerRegistered } from './leaderboard.js';
import { DICE_SKINS, CASE_DROPPABLE_SKINS, CASE_PRICE, DUPLICATE_COINS_REFUND, getDiceSkin, rollDiceSkinFromCase, applyDiceSkinToElement, create2DDiceHTML, create3DDiceHTML, getDiceFaceRotations } from './dice-skins.js';
import { sendMatchFinishedWebhook } from './webhook.js';
import { TITLES, getTitleById, formatTitleBadge, getTitleProgressRatio } from './titles.js';
import { pawnEditor } from './pawn-editor.js';
import { matchHistoryManager } from './history.js';
import { MAP_THEMES, getMapThemeById, getThemedTileData } from './map-themes.js';
import { generateThematicTiles } from './city-generator.js';
...
class App {
  ...
  init() {
    ...
    // Check for room in URL (?room=XYZ) or saved session
          ...
          <div class="player-info-left">
            ...
            <div>
              <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                <span class="player-name-tag" style="color: ${hasCustomBg ? bgDef.textColor : p.color};">
    ...
    switch (payload.type) {
      ...
      case 'ADMIN_COIN_GRANT': {
        if (payload.targetPlayerId && String(payload.targetPlayerId) === String(profileManager.profile.id)) {
          ...
          this.renderLeaderboard();
          sound.playCash();
          ui.showToast(`👑 Администратор ${payload.actionType === 'take' ? 'списал' : 'выдал'} вам ${payload.amount} 🪙`);
        }
        break;
      }
    }
  ...
  renderProfileCard() {
    ...
    if (cardName) {
      cardName.innerText = isRegistered ? p.name : 'Гость';
    }
    ...
    if (isRegistered) {
      ...
      if (headerName) headerName.innerHTML = `${renderTokenHTML(p.token, p.customToken, 'token-custom-img')} Гость`;
    }
  ...
  renderLeaderboard(sortBy = 'wins', containerId = 'inline-leaderboard-list') {
    ...
    const isUserRegistered = profileManager.isRegisteredUser();
    ...
    if (!isUserRegistered) {
      bannerHtml = `
        <div class="lb-guest-banner">
          <div class="lb-guest-banner-text">
            <b>В рейтинге только подтверждённые игроки Discord.</b><br>
            Войдите в аккаунт, чтобы попасть в официальный топ!
          </div>
          <button class="md-btn discord-brand-btn btn-trigger-discord-auth" style="padding: 6px 12px; font-size: 0.78rem;">
            Войти
          </button>
        </div>
      `;
    ...
    let html = bannerHtml + list.map(p => {
      let scoreText = '';
      if (sortBy === 'cash') {
      ...
      return `
        <div class="leaderboard-item ${p.isMe ? 'is-me' : ''}" data-player-id="${p.id}" style="cursor: pointer;" title="Нажмите, чтобы просмотреть профиль игрока">
          <div class="lb-left">
            ...
            <span class="lb-token" style="display: inline-flex; align-items: center; justify-content: center; width: 24px; height: 24px;">${renderTokenHTML(p.token, p.customToken)}</span>
            <div class="lb-info">
              <div class="lb-name">
    ...
    if (list.length === 0) {
      html += `
        <div class="lb-empty-notice">
          ...
          <span>В официальном топе пока нет подтверждённых игроков. Войдите через Discord и займите первое место!</span>
        </div>
      `;
    }
  ...
  openDiceCase() {
    const coins = profileManager.profile.coins || 0;
    if (coins < CASE_PRICE) {
      ui.showToast(`Недостаточно монет! Требуется ${CASE_PRICE} 🪙, у вас ${coins} 🪙`);
      return;
    }
  ...
  initAdminListeners() {
    ...
    document.getElementById('form-admin-set-my-coins')?.addEventListener('submit', (e) => {
      ...
      if (input) {
        ...
        this.renderAdminModal();
        this.renderLeaderboard();
        ui.showToast(`Баланс установлен: ${val} 🪙`);
      }
    });
  ...
  renderAdminModal() {
    ...
    if (coinsDisplay) {
      coinsDisplay.innerText = `${(p.coins || 0).toLocaleString('ru-RU')} 🪙`;
    }
    ...
    select.innerHTML = Array.from(playerMap.values()).map(pl => {
      const label = pl.isMe ? `[Вы] ${pl.name} (${(pl.coins || 0)} 🪙)` : `${pl.name} (${(pl.coins || 0)} 🪙) — ID: ${String(pl.id).substring(0, 10)}`;
  ...
  handleAdminPlayerCoinAdjustment(isGive) {
    ...
    if (isMe) {
      if (isGive) {
        profileManager.addCoins(amount);
        sound.playCash();
        ui.showToast(`Выдано себе: +${amount} 🪙`);
      } else {
        profileManager.removeCoins(amount);
        sound.playClick();
        ui.showToast(`Списано у себя: -${amount} 🪙`);
      }
      this.renderProfileCard();
      this.renderAdminModal();
      this.renderLeaderboard();
    ...
    sound.playCash();
    ui.showToast(`👑 ${isGive ? 'Выдано' : 'Списано'} ${amount} 🪙 игроку (ID: ${targetId})`);
    this.renderAdminModal();
  }
        ...
        <div class="match-history-card ${m.isMeWinner ? 'is-win' : 'is-defeat'}">
          ...
          <div class="mh-opponents-row">
            <div>
              ...
              ${winnerToken} <b style="color:${m.winner.color || '#38bdf8'};">${m.winner.name}</b>
            </div>
  ...
  handleGameFinished(state) {
    ...
    if (profileManager.isRegisteredUser() && network) {
      network.broadcastLeaderboardRecord({
        id: profileManager.profile.id,
        name: profileManager.profile.name,
        token: profileManager.profile.token,
